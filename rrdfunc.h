...
extern char *rrdCreateViaDaemon(int argc, char **argv);
extern char *rrdUpdate(const char *filename, const char *template, int argc, const char **argv);
extern char *rrdUpdateDaemon(int argc, const char **argv);
...
extern char *rrdFetchDaemon(int *ret, int argc, char **argv, time_t *start, time_t *end, unsigned long *step, unsigned long *ds_cnt, char ***ds_namv, double **data);
